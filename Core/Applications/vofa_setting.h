#ifndef _VOFA_SETTING_H_
#define _VOFA_SETTING_H_

#ifdef __cplusplus
extern "C" {
#endif




/*
修改VOFASendLen选择发送数据的长度
修改VOFAClearTime选择超时清空的时间（根据定时调用的时间更改）
修改VOFAClearLen选择超长丢弃的长度（一般不用改）
*/

/*#include "vofa.h"*/
#define VOFA_USE_DMA 1//为1时vofa通过dma传输
#define VOFA_SEND_NUM 75
#define TEMP_FLOAT_NUM 75

extern float VofaData[32];//需要赋给控件的变量extern过来
//extern float tempFloat[TEMP_FLOAT_NUM];//发送数据暂存

extern float VofaButton[32];//需要赋给控件的变量extern过来
extern float VofaKey[32];
extern float VofaSlider[32];
extern float tempFloat[TEMP_FLOAT_NUM];//发送数据暂存

#define VOFASendLen 16    //定义发送信号长度
#define VOFAClearTime 50 //定义超时清空时间  = VOFAOutTime*一次中断时间(0~255)
#define VOFAClearLen 20  //定义超长丢弃长度  (0~63)

#define UseVOFASlider1
#define UseVOFASlider2
#define UseVOFASlider3
#define UseVOFASlider4
#define UseVOFASlider5
#define UseVOFASlider6
#define UseVOFASlider7

#define UseVOFAButton1
#define UseVOFAButton2
#define UseVOFAButton3
#define UseVOFAButton4
#define UseVOFAButton5
#define UseVOFAButton6
#define UseVOFAButton7
#define UseVOFAButton8

#define UseVOFAKey1     //START
#define UseVOFAKey2
#define UseVOFAKey3
#define UseVOFAKey4

//#define UseVOFABar1
//#define UseVOFABar2

#ifdef UseVOFASlider1
#define Vofa_Slider1 VofaSlider[0]
#endif
#ifdef UseVOFASlider2
#define Vofa_Slider2 VofaSlider[1]
#endif
#ifdef UseVOFASlider3
#define Vofa_Slider3 VofaSlider[2]
#endif
#ifdef UseVOFASlider4
#define Vofa_Slider4 VofaSlider[3]
#endif
#ifdef UseVOFASlider5
#define Vofa_Slider5 VofaSlider[4]
#endif
#ifdef UseVOFASlider6
#define Vofa_Slider6 VofaSlider[5]
#endif
#ifdef UseVOFASlider7
#define Vofa_Slider7 VofaSlider[6]
#endif

#ifdef UseVOFAButton1
#define Vofa_Button1  VofaButton[0]
#endif
#ifdef UseVOFAButton2
#define Vofa_Button2  VofaButton[1]
#endif
#ifdef UseVOFAButton3
#define Vofa_Button3  VofaButton[2]
#endif
#ifdef UseVOFAButton4
#define Vofa_Button4  VofaButton[3]
#endif
#ifdef UseVOFAButton5
#define Vofa_Button5  VofaButton[4]
#endif
#ifdef UseVOFAButton6
#define Vofa_Button6  VofaButton[5]
#endif
#ifdef UseVOFAButton7
#define Vofa_Button7  VofaButton[6]
#endif
#ifdef UseVOFAButton8
#define Vofa_Button8  VofaButton[7]
#endif

#ifdef UseVOFAKey1
#define Vofa_Key1     VofaKey[0]
#endif
#ifdef UseVOFAKey2
#define Vofa_Key2       VofaKey[1]
#endif
#ifdef UseVOFAKey3
#define Vofa_Key3       VofaKey[2]
#endif
#ifdef UseVOFAKey4
#define Vofa_Key4       VofaKey[3]
#endif

#ifdef UseVOFABar1
#define Vofa_Bar_x1
#define Vofa_Bar_y1
#endif
#ifdef UseVOFABar2
#define Vofa_Bar_x2
#define Vofa_Bar_y2
#endif



#ifdef __cplusplus
}
#endif
#endif
